"""
TSRN-Gist syndrome decoder: train on toric code syndrome data, report logical error rates.

Uses the TSRNGist model (Clifford rotor gists, sheaf rotor diffusion, RG pooling)
instead of the base TSRN model. Otherwise identical evaluation protocol to
tsrn_syndrome.py for direct comparison.

Trains one TSRNGist model per code distance (d=3, 5, 7).
Evaluates at multiple physical error rates for direct comparison to published MWPM.

Usage:
  python tsrn_syndrome_gist.py --steps 20000 --d-model 256 --n-blocks 2
"""
...
from tsrn_dml import detect_device, get_lr
...
def train_decoder(dataset: SyndromeDataset, device, d: int,
                  ...
                  top_k: int = 8, mem_depth: int = 5,
                  sheaf_window: int = 3, max_gists: int = 64,
                  gist_top_k: int = 4, dropout: float = 0.1,
                  ckpt_every: int = 2000, resume_from: str = None,
                  gradient_checkpoint: bool = False):
    """Train TSRNGist decoder for a single code distance."""

    V = dataset.vocab_sz
    ctx = dataset.seq_len  # exact sequence length (d^2 + 1)

    # TSRNGist uses RG pooling (T -> T/2), context_len must be even
    assert ctx % 2 == 0, f"context_len={ctx} must be even for TSRNGist RG pooling"

    torch.manual_seed(42)
    model = TSRNGist(vocab=V, d_model=d_model, context_len=ctx,
                     gradient_checkpoint=gradient_checkpoint,
                     n_blocks=n_blocks, top_k=top_k, n_heads=n_heads,
                     mem_depth=mem_depth, sheaf_window=sheaf_window,
    ...
    print(f"\n  TSRNGist Syndrome Decoder (d={d}): {n_params:,} params")
    print(f"  Vocab: {V}  |  SeqLen: {ctx}  |  d_model: {d_model}  |  Blocks: {n_blocks}")
    print(f"  SheafWin: {sheaf_window}  |  Gists: {max_gists}  |  GistTopK: {gist_top_k}")
    ...
    print(f"\n{'='*80}")
    print(f"  Toric Code d={d} — TSRNGist Syndrome Decoder Training — {n_steps} steps")
    print(f"{'='*80}")
    ...
    best_ckpt = f"checkpoints/tsrn_gist_syndrome_d{d}_best.pt"
    torch.save({
        "model_state_dict": model.state_dict(),
        "config": {"vocab": V, "d_model": d_model, "context_len": ctx,
                   "n_blocks": n_blocks, "top_k": top_k,
    ...
    sweep = evaluate_at_error_rates(model, d, device,
                                     ...
                                     n_samples=10_000)
    print(f"  {'p':>6}  {'Raw p_L':>8}  {'Gist p_L':>9}  {'Acc':>7}")
    print(f"  {'-'*35}")
    ...
    results = {
        ...
        "config": {
            ...
            "sheaf_window": sheaf_window, "max_gists": max_gists,
...
def main():
    parser = argparse.ArgumentParser(description="TSRNGist Syndrome Decoder")
    ...
    parser.add_argument("--sheaf-window", type=int, default=3)
    ...
    for d in distances:
        print(f"\n{'#'*80}")
        print(f"  TORIC CODE d={d} — Training TSRNGist Syndrome Decoder")
        print(f"{'#'*80}")
        ...
        resume_path = None
        if args.resume:
            resume_path = f"checkpoints/tsrn_gist_syndrome_d{d}_best.pt"
        ...
        model, results = train_decoder(
            ...
            sheaf_window=args.sheaf_window,
        ...
        out = f"results/tsrn_gist_syndrome_d{d}_{args.steps}steps.json"
        with open(out, "w") as f:
            json.dump(results, f, indent=2)
        print(f"\n  Results saved: {out}")

    # Save combined results
    combined_out = f"results/tsrn_gist_syndrome_combined_{args.steps}steps.json"
    ...
    # Print summary table
    print(f"\n{'='*80}")
    print(f"  SUMMARY: TSRNGist Syndrome Decoder — Toric Code Bit-Flip")
    print(f"{'='*80}")
    ...
    if args.mwpm:
        ...
        print(f"\n  TSRNGist vs MWPM at p=0.05 (training error rate):")
        print(f"  {'d':>3}  {'MWPM':>8}  {'Gist val':>10}  {'Gist test':>10}  {'Winner':>8}")
        print(f"  {'-'*48}")
        for d in distances:
            mwpm_acc = mwpm_results[d][0.05]['accuracy']
            tsrn_val = all_results[d]['best_val_accuracy']
            tsrn_test = all_results[d]['test']['accuracy']
            winner = "Gist" if tsrn_test > mwpm_acc else "MWPM"
            print(f"  {d:>3}  {mwpm_acc:>7.1%}  {tsrn_val:>9.1%}  {tsrn_test:>9.1%}  {winner:>8}")

        # Save MWPM results
        mwpm_out = f"results/mwpm_baseline_gist.json"
