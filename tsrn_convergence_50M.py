"""
TSRN 100k-step convergence run on enwik8 (50M parameter model).
- TSRN with 48.3M parameters (d_model=512, n_blocks=7, n_heads=8)
...
def train_convergence(dataset, device, n_steps=100000, batch_size=8,
    ...
    print(f"\n{'='*80}")
    print(f"  TSRN 100k-step Run — enwik8 byte-level (50M params)")
    print(f"{'='*80}")
    ...
    for step in range(start_step + 1, n_steps + 1):
        ...
        if step % ckpt_every == 0:
            ...
            ckpt_path = f"checkpoints/tsrn_enwik8{sfx}_{step}steps.pt"
            torch.save({
